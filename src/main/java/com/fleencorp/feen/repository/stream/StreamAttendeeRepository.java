...
public interface StreamAttendeeRepository extends JpaRepository<StreamAttendee, Long> {
  ...
  List<StreamAttendee> findAllByStreamAndRequestToJoinStatus(FleenStream fleenStream, StreamAttendeeRequestToJoinStatus requestToJoinStatus);
  ...
  Page<StreamAttendee> findByStream(FleenStream fleenStream, Pageable pageable);

  @Query(value = "SELECT sa FROM StreamAttendee sa WHERE sa.stream = :fleenStream AND sa.stream.streamType = :streamType")
  Page<StreamAttendee> findByFleenStreamAndStreamType(FleenStream fleenStream, StreamType streamType, Pageable pageable);

  Page<StreamAttendee> findByStreamAndRequestToJoinStatus(FleenStream fleenStream, StreamAttendeeRequestToJoinStatus requestToJoinStatus, Pageable pageable);

  Page<StreamAttendee> findAllByStreamAndRequestToJoinStatusAndAttending(FleenStream fleenStream, StreamAttendeeRequestToJoinStatus requestToJoinStatus, Boolean isAttending, Pageable pageable);

  long countByStreamAndRequestToJoinStatusAndAttending(FleenStream fleenStream, StreamAttendeeRequestToJoinStatus requestToJoinStatus, Boolean isAttending);
  ...
  @Query("SELECT sa FROM StreamAttendee sa WHERE sa.stream = :fleenStream AND sa.member = :member")
  Optional<StreamAttendee> findAttendeeByStreamAndUser(@Param("fleenStream") FleenStream fleenStream, @Param("member") Member member);

  @Query("SELECT sa FROM StreamAttendee sa WHERE sa.stream = :fleenStream AND sa.attending = true")
