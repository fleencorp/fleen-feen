...
public interface StreamAttendeeRepository extends JpaRepository<StreamAttendee, Long> {
  ...
  @Query("SELECT sa FROM StreamAttendee sa WHERE sa.fleenStream.streamId = :streamId AND sa.member.memberId IN (:memberIds) AND sa.requestToJoinStatus IN (:statuses)")
  ...
  @Query("SELECT new com.fleencorp.feen.model.projection.StreamAttendeeSelect(fs.streamId, sa.requestToJoinStatus, sa.attending, sa.fleenStream.streamVisibility, sa.fleenStream.scheduledEndDate) " +
    "FROM StreamAttendee sa LEFT JOIN sa.member m LEFT JOIN sa.fleenStream fs WHERE m = :member AND fs.streamId IN (:ids)")
